"""Decline, when there is nothing to answer from.

The composing half used to live here and now lives in `council/narrate.py`,
outside the graph - a live generator in state is not serialisable, and a graph
that puts one there cannot be checkpointed. What is left is the declining path,
which was never an error path: the corpus being silent on a question is a
legitimate answer, and composing prose over nothing retrieved is the failure the
whole grounding discipline exists to prevent.
"""
